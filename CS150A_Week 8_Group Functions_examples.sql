...
/* Example 5: Find the sum of the salaries of all employees, the maximum salary, the minimum salary, and 
   the average salary.*/
SELECT SUM(salary) AS sum_salary, MAX(salary) AS max_salary
       ,MIN(salary) AS min_salary, AVG(salary) AS avg_salary
FROM employee;

/* Example 6: The following query displays the minimum and maximum salary of all departments.
...
/* Example 7: For each department, retrieve the department name, the number of employees in the 
   department, and their average salary. We write this query as follows:*/
...
/* Example 8: For each project, list the project name and the total hours per week (by all employees) spent 
   on that project.*/
...
/* Example 9: For each department, retrieve the gender, the number of employees in the company, and their 
   average salary.*/
...
/* Example 12: For each project on which more than two employees work, retrieve the project number, the 
...
/* Example 13: Display the total number of dependents for each employee for employees who have at least 
  two dependents. */
SELECT essn, COUNT(*)
FROM dependent
GROUP BY essn 
...
/*Example 14: Display average employee salary by department, but do not include departments with 
  average salary less than or equal $50,000.*/
...
/* Example 16: What is the total MaxHours of projects being run by the Research Department? Be 
...
/* Example 17: How many projects are being run by each department? Be sure to display each 
...
/* Example 18: Write an SQL statement that shows each type (that is, dog or cat) and the number of each *
...
/* Example 19: Write an SQL statement to display each Breed of dog and the number of each Breed in the database. */
...
/* Example 20: Write an SQL statement to group the data by PetBreed and display the average Weight per breed.*/
SELECT PetBreed, AVG(PetWeight) AS AvgBreedWeight
FROM pet_3
GROUP BY PetBreed;

/* Example 21: Write an SQL statement to group the data by PetBreed and display the average Weight per
